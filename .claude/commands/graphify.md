---
description: Turn any folder of files (code, docs, papers, images, video) into a navigable knowledge graph with community detection, an honest audit trail, and interactive HTML + JSON + GRAPH_REPORT.md outputs.
argument-hint: "[path | github-url | query \"<question>\" | path \"<A>\" \"<B>\" | explain \"<concept>\"] [--mode deep] [--update] [--wiki] [--obsidian] [--no-viz]"
---

<!--
  MAINTENANCE NOTE
  This command carries the full pipeline spec so it works standalone. The canonical
  copy is .claude/skills/graphify/SKILL.md. The bodies are identical except that
  reference paths here are absolute-from-repo-root (.claude/skills/graphify/references/…)
  because commands/ is a sibling of skills/, not inside it.

  When you update one, update the other in the same commit. Otherwise /graphify
  invoked as a command and /graphify invoked as a skill will behave differently —
  which is exactly the drift that was flagged when this file was requested.
-->
...
Only when the path is one or more `https://github.com/...` URLs, or several local subfolders to merge. See `.claude/skills/graphify/references/github-and-merge.md` for the clone, cross-repo merge, and monorepo flow, then continue with the resolved local path. A plain local path skips this step.
...
Skip this step entirely if `detect` returned zero `video` files. When the corpus has video or audio, see `.claude/skills/graphify/references/transcribe.md` to transcribe them to text first, then treat the transcripts as doc files in Step 3.
...
Both are non-default subcommands. `--update` re-extracts only new or changed files; `--cluster-only` reruns clustering on the existing graph. See `.claude/skills/graphify/references/update.md` for both flows.
...
Answer using only what the graph output contains, and quote `source_location` when citing a specific fact. Before traversal, expand the question against the graph's own vocabulary so a wording mismatch does not collapse the answer to noise. For that vocab-expansion step, the `--dfs` / `--budget` modes, `save-result` feedback, and the `/graphify path` and `/graphify explain` flows, see `.claude/skills/graphify/references/query.md`.

---

## For /graphify add and --watch

Neither is part of the default build. When the user runs `/graphify add <url>` to fetch a URL into the corpus, or passes `--watch` to auto-rebuild on file changes, see `.claude/skills/graphify/references/add-watch.md`.

---

## For the commit hook and native CLAUDE.md integration

When the user asks to install the post-commit auto-rebuild hook or wire graphify into a project's CLAUDE.md, see `.claude/skills/graphify/references/hooks.md`.
