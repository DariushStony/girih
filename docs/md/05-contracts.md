...
**components/button.spec.ts  [You write this]**
...
| # | Stage | What happens |
| --- | --- | --- |
| 1 | **Load** | jiti imports the .spec.ts |
...
1. **Multiple targets, one shape.** A future Figma generator should consume one  canonical form rather than re-reading and re-interpreting `.spec.ts` files. The React  generator already only sees IR — it has no idea TypeScript was involved.
...
```tsx
/* Generated by girih — do not edit. Source of truth: components/button.spec.ts */
