...
${table(
  ['You want to', 'Go to'],
  [
    ['Add a variant, size or state to a component', '<a href="#anatomy">Anatomy</a> — every field, and what each one does to the output'],
    [
      'Work out why a token reference fails to validate',
      '<a href="#validation">Cross-validation</a> — especially if it only fails for one brand',
    ],
    ['Vary a component without forking it', '<a href="#extensions">Extensions</a> — <code>defineVariant</code> and its limits'],
    ['Hand-write the markup for one component', '<a href="#eject">Ejection</a> — forking as a recorded operation'],
    ['Understand a version bump you did not expect', '<a href="#semver">Versioning</a> — what feeds the signature'],
    ['Know why <code>.ds/ir/</code> is committed to your repo', '<a href="#ir">The IR</a> — and why it is deleted on every generate'],
  ],
)}
...
<p>
  There are ${data.families['4'].codes.length} codes in the <code>GIRIH4xxx</code> range — more than
  any other family — because a contract has the largest surface area of anything a user writes. The
  full list with messages is in <a href="07-error-codes.html">chapter 07</a>.
</p>

${strap()}

<p class="lede">
  Everything above is what a contract <em>is</em> and what girih makes of it. The rest of this chapter
