...
This project has a knowledge graph at `graphify-out/` with god nodes, community structure, and cross-file relationships (1,990 nodes · 2,692 edges · 153 communities, built from 245 files).
...
- **Contracts:** `examples/acme-ds/components/<name>.contract.ts` (default-exported `defineSpec`), extensions in `extensions/<name>.ext.ts` (default-exported `defineVariant`). Filenames are kebab-case; the `name` field is PascalCase and drives the emitted component name.
